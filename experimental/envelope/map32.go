...
func NewLerpMap32[TIn, TOut lerp.Lerpable](points []MapEntry[TIn, TOut]) Map[TIn, TOut] {
	if len(points) == 0 {
...
func NewPointMap32[TIn, TOut lerp.Lerpable](points []MapEntry[TIn, TOut]) Map[TIn, TOut] {
	if len(points) == 0 {
		panic("must have at least 1 point")
	}
	...
	for _, cur := range p {
		rem = append(rem, remapList[TIn, TOut, float32]{
			Remapper: lerp.NewRemapPoint[TIn, TOut, float32](cur.Input, cur.Output),
		})
	}
	last := &p[len(p)-1]
