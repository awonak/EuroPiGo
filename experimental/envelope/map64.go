...
type envMap64[TIn, TOut lerp.Lerpable] struct {
	rem    []lerp.Remapper64[TIn, TOut]
	outMax TOut
}
...
func NewMap64[TIn, TOut lerp.Lerpable](points []MapEntry[TIn, TOut]) Map[TIn, TOut] {
	...
	var outMax TOut
	var rem []lerp.Remapper64[TIn, TOut]
	if len(p) == 1 {
		cur := p[0]
		outMax = cur.Output
		rem = append(rem, lerp.NewRemapPoint[TIn, TOut, float64](cur.Input, cur.Output))
	} else {
		for pos := 0; pos < len(p)-1; pos++ {
			cur, next := p[pos], p[pos+1]
			outMax = next.Output
			rem = append(rem, lerp.NewRemap64(cur.Input, next.Input, cur.Output, next.Output))
		}
	}
...
func (m *envMap64[TIn, TOut]) Remap(value TIn) TOut {
	for _, r := range m.rem {
...
func (m *envMap64[TIn, TOut]) InputMinimum() TIn {
	// we're guaranteed to have 1 point
	return m.rem[0].InputMaximum()
}

func (m *envMap64[TIn, TOut]) InputMaximum() TIn {
	// we're guaranteed to have 1 point
	return m.rem[len(m.rem)-1].InputMaximum()
}

func (m *envMap64[TIn, TOut]) OutputMinimum() TOut {
	// we're guaranteed to have 1 point
	return m.rem[0].OutputMinimum()
}

func (m *envMap64[TIn, TOut]) OutputMaximum() TOut {
	return m.outMax
}
