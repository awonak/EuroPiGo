...
func init() {
	...
	hwDigital1Input = newDigitalInput(newPicoDigitalReader(machine.GPIO22))
	hwAnalog1Input = newAnalogInput(newPicoAdc(machine.ADC0))
	hwDisplay1Output = newDisplayOutput(newPicoDisplayOutput(machine.I2C0, machine.GPIO0, machine.GPIO1))
	hwButton1Input = newDigitalInput(newPicoDigitalReader(machine.GPIO4))
	hwButton2Input = newDigitalInput(newPicoDigitalReader(machine.GPIO5))
	hwKnob1Input = newAnalogInput(newPicoAdc(machine.ADC1))
	hwKnob2Input = newAnalogInput(newPicoAdc(machine.ADC2))
	hwCV1Output = newVoltageOuput(newPicoPwm(machine.PWM2, machine.GPIO21))
	hwCV2Output = newVoltageOuput(newPicoPwm(machine.PWM2, machine.GPIO20))
	hwCV3Output = newVoltageOuput(newPicoPwm(machine.PWM0, machine.GPIO16))
	hwCV4Output = newVoltageOuput(newPicoPwm(machine.PWM0, machine.GPIO17))
	hwCV5Output = newVoltageOuput(newPicoPwm(machine.PWM1, machine.GPIO18))
	hwCV6Output = newVoltageOuput(newPicoPwm(machine.PWM1, machine.GPIO19))
	hwRandom1Generator = newRandomGenerator(&picoRnd{})
}
